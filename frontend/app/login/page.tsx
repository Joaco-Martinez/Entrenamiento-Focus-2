...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (!authLoading && isAuth) {
      // si ya está logueado y no vino redirect, lo mandamos a dashboard
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // prioridad: redirect (si vino)
      if (redirectTo) return router.replace(redirectTo)

      // sino: role-based
      if (data.user.role === "admin") return router.replace("/admin")
  ...
  return (
    <div className="min-h-screen bg-[#0B0B0B] text-white">
      ...
      <main className="relative z-10 mx-auto grid max-w-6xl items-center gap-10 px-6 py-16 md:grid-cols-2">
        {/* Bloque título estilo hero */}
        <div>
          <h1 className="text-4xl font-extrabold leading-tight md:text-6xl">
            Iniciá sesión y{" "}
            <span className="text-yellow-400">accedé</span>
            <br />
            a tu{" "}
            <span className="text-yellow-400">contenido</span>
          </h1>
          ...
          <p className="mt-6 max-w-xl text-base leading-relaxed text-white/70">
            Entrá con tu cuenta para ver cursos, recursos y beneficios.
            Si tenés suscripción activa, vas a ver el contenido premium automáticamente.
          </p>
