...
export async function mpSubscription(req: Request, res: Response) {
  ...
  // IMPORTANTE:
  // Si mercadoPagoCreateSubscription acepta solo 2 args,
  // dejalo así:
...
export async function paypalCheckout(req: Request, res: Response) {
  const r = await paymentsService.paypalCreateCheckout(
    req.body.orderId,
    req.body.returnUrl,
    req.body.cancelUrl
  );
...
export async function paypalCapture(req: Request, res: Response) {
  ...
  const r = await paymentsService.paypalCaptureCheckout(
    userId,
